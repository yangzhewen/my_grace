...
video_filter = "video.str.contains('video')"
df_grace = read_df("grace/all.csv").query(video_filter).drop(columns=["video"])
# print(df_grace["video"].unique())
df_265 = read_df("h265/all.csv").query(video_filter).drop(columns=["video"])
df_264 = read_df("h264/all.csv").query(video_filter).drop(columns=["video"])
df_pretrain = read_df("pretrained/all.csv").query(video_filter).drop(columns=["video"])
df_error = read_df("error_concealment/all.csv").query(video_filter).drop(columns=["video"])
